...
public abstract class AbstractSQL<T> {
    ...
    public T PRIMARY_KEY(String key) {
        sql().columns.add("  PRIMARY KEY  (" + key + ")");
        return getSelf();
    }

    public T INDEX(String... key) {
        for (int i = 0; i < key.length; i++) {
            sql().columns.add("  INDEX " + key[i] + "_index  (" + key[i] + ")");
        }
        return getSelf();
    }
    ...
    public T CREATE(String table) {
        sql().statementType = SQLStatement.StatementType.CREATE;
        sql().tables.add(table);
        return getSelf();
    }
    ...
    private static class SQLStatement {
        ...
        public enum StatementType {
            DELETE, INSERT, SELECT, UPDATE, CREATE
        }
        ...
        private String createSQL(SafeAppendable builder) {
            sqlClause(builder, "CREATE TABLE IF NOT EXISTS", tables, "", "", "");
            sqlClause(builder, "", columns, "(", ");", ", ");
            return builder.toString();
        }
